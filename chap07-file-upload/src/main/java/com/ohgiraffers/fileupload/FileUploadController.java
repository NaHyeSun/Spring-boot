...
public class FileUploadController {
    ...
    public String singleFileUpload(@RequestParam String singleFileDescription
        ...
        System.out.println(dir.getAbsolutePath());  //경로 출력

        if(!dir.exists()) dir.mkdirs();  //경로가 없으면 만들어라

        /* 파일명 변경 처리 */
        String originFileName = singleFile.getOriginalFilename(); //원래 파일 이름
        String ext = originFileName.substring(originFileName.lastIndexOf(".")); // 확장자만 분리 시켜서 잘라냄
        String savedName = UUID.randomUUID() + ext;     //UUID : 유니크한 랜덤한 아이디 만들어줌

        /* 파일 저장 */
        try {
            singleFile.transferTo(new File(filePath + "/" + savedName)); //지정된 경로쪽으로 보내줌
